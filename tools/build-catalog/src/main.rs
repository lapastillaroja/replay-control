...
#[derive(Parser)]
#[command(name = "build-catalog", about = "Build catalog.sqlite from game data files")]
struct Args {
...
fn parse_fbneo_dat(path: &Path) -> Vec<ArcadeEntry> {
    ...
    let mut reader = match Reader::from_file(path) {
        Ok(r) => r,
        Err(e) => {
            eprintln!("Warning: failed to open FBNeo DAT at {}: {}", path.display(), e);
            return entries;
        }
    };
    ...
    loop {
        match reader.read_event_into(&mut buf) {
                    ...
                    b"game" => {
                        ...
                        for attr in e.attributes().filter_map(|a| a.ok()) {
                            match attr.key.local_name().as_ref() {
                                ...
                                b"cloneof" => current_cloneof = String::from_utf8_lossy(&attr.value).into_owned(),
                                b"isbios" => current_is_bios = String::from_utf8_lossy(&attr.value) == "yes",
                                _ => {}
                            }
                        }
                    }
            ...
            Ok(Event::Eof) => break,
            Err(e) => {
                eprintln!("Error parsing FBNeo DAT at {}: {:?}", reader.error_position(), e);
                break;
            }
            _ => {}
        }
...
fn parse_mame2003plus_xml(path: &Path) -> Vec<ArcadeEntry> {
    ...
    let mut reader = match Reader::from_file(path) {
        Ok(r) => r,
        Err(e) => {
            eprintln!("Warning: failed to open MAME 2003+ XML at {}: {}", path.display(), e);
            return entries;
        }
    };
    reader.config_mut().trim_text(true);
    ...
    loop {
        match reader.read_event_into(&mut buf) {
                    ...
                    b"game" => {
                        ...
                        for attr in e.attributes().filter_map(|a| a.ok()) {
                            match attr.key.local_name().as_ref() {
                                ...
                                b"cloneof" => current_cloneof = String::from_utf8_lossy(&attr.value).into_owned(),
                                b"runnable"
                                    if String::from_utf8_lossy(&attr.value) == "no" => {
                    ...
                    b"input" => {
                        for attr in e.attributes().filter_map(|a| a.ok()) {
                            if attr.key.local_name().as_ref() == b"players" {
                                current_players = String::from_utf8_lossy(&attr.value).parse().unwrap_or(0);
                            }
                        }
                    }
                    b"driver" => {
            ...
            Ok(Event::Eof) => break,
            Err(e) => {
                eprintln!("Error parsing MAME 2003+ XML at {}: {:?}", reader.error_position(), e);
                break;
            }
            _ => {}
        }
...
fn parse_mame_current_xml(path: &Path) -> Vec<ArcadeEntry> {
    ...
    let mut reader = match Reader::from_file(path) {
        Ok(r) => r,
        Err(e) => {
            eprintln!("Warning: failed to open MAME current XML at {}: {}", path.display(), e);
            return entries;
        }
    };
    reader.config_mut().trim_text(true);
    ...
    loop {
        match reader.read_event_into(&mut buf) {
                    ...
                    b"m" => {
                        ...
                        for attr in e.attributes().filter_map(|a| a.ok()) {
                            match attr.key.local_name().as_ref() {
                                b"name" => current_name = String::from_utf8_lossy(&attr.value).into_owned(),
                                b"cloneof" => current_cloneof = String::from_utf8_lossy(&attr.value).into_owned(),
                                b"rotate" => current_rotate = String::from_utf8_lossy(&attr.value).into_owned(),
                                b"players" => current_players = String::from_utf8_lossy(&attr.value).parse().unwrap_or(0),
                                b"status" => current_status = String::from_utf8_lossy(&attr.value).into_owned(),
                                _ => {}
                            }
                        }
                    }
            ...
            Ok(Event::Eof) => break,
            Err(e) => {
                eprintln!("Error parsing MAME current XML at {}: {:?}", reader.error_position(), e);
                break;
            }
            _ => {}
        }
...
fn parse_catver_ini(path: &Path) -> HashMap<String, String> {
    ...
    let file = match File::open(path) {
        Ok(f) => f,
        Err(e) => {
            eprintln!("Warning: failed to open catver.ini at {}: {}", path.display(), e);
            return categories;
        }
    };
    ...
    for line in BufReader::new(file).lines() {
        ...
        if in_category_section
            && let Some((rom_name, category)) = trimmed.split_once('=') {
...
fn parse_nplayers_ini(path: &Path) -> HashMap<String, u8> {
    ...
    let file = match File::open(path) {
        Ok(f) => f,
        Err(e) => {
            eprintln!("Warning: failed to open nplayers.ini at {}: {}", path.display(), e);
            return players_map;
        }
    };
...
fn insert_arcade_games(conn: &Connection, sources_dir: &Path) -> rusqlite::Result<()> {
    ...
    if flycast_path.exists() {
        ...
        eprintln!("Arcade DB: Flycast CSV loaded {} entries", flycast_entries.len());
    ...
    if fbneo_path.exists() {
        ...
        eprintln!("Arcade DB: FBNeo DAT loaded {} entries", fbneo_entries.len());
        for entry in fbneo_entries {
            let rom_name = entry.rom_name.clone();
            if let std::collections::hash_map::Entry::Vacant(v) = entries_map.entry(rom_name.clone()) {
                v.insert(entry);
    ...
    if mame_path.exists() {
        ...
        eprintln!("Arcade DB: MAME 2003+ loaded {} entries", mame_entries.len());
    ...
    if mame_current_path.exists() {
        ...
        eprintln!("Arcade DB: MAME current loaded {} entries", mame_current_entries.len());
        ...
        eprintln!("Arcade DB: MAME current added {} new, overrode {} existing", new_count, override_count);
    }
    ...
    if catver_current_path.exists() {
        ...
        eprintln!("Arcade DB: Applied {} catver-mame-current.ini overlays", applied);
    }
    ...
    let non_game_prefixes = [
        "Electromechanical", "Slot Machine", "Gambling", "Computer",
        "Handheld", "Game Console", "Calculator", "Printer", "Utilities", "System",
    ];
    ...
    entries_map.retain(|_, entry| {
        ...
        !non_game_prefixes.iter().any(|p| entry.category.starts_with(p))
    });
    eprintln!("Arcade DB: Filtered {} non-game machines", total_before - entries_map.len());
    ...
    let mut stmt_rd = conn.prepare(
        "INSERT INTO arcade_release_dates (rom_name, year, source) VALUES (?1, ?2, ?3)"
    )?;
...
const GAME_DB_SYSTEMS: &[SystemConfig] = &[
    ...
    SystemConfig { folder_name: "nintendo_nes",  nointro_dat: "Nintendo - Nintendo Entertainment System.dat", tgdb_platform_ids: &[7] },
    SystemConfig { folder_name: "nintendo_snes", nointro_dat: "Nintendo - Super Nintendo Entertainment System.dat", tgdb_platform_ids: &[6] },
    SystemConfig { folder_name: "nintendo_gb",   nointro_dat: "Nintendo - Game Boy.dat",           tgdb_platform_ids: &[4] },
    SystemConfig { folder_name: "nintendo_gbc",  nointro_dat: "Nintendo - Game Boy Color.dat",     tgdb_platform_ids: &[41] },
    SystemConfig { folder_name: "nintendo_gba",  nointro_dat: "Nintendo - Game Boy Advance.dat",   tgdb_platform_ids: &[5] },
    SystemConfig { folder_name: "nintendo_n64",  nointro_dat: "Nintendo - Nintendo 64.dat",        tgdb_platform_ids: &[3] },
    // Sega cartridge/handheld
    SystemConfig { folder_name: "sega_sms",  nointro_dat: "Sega - Master System - Mark III.dat", tgdb_platform_ids: &[35] },
    SystemConfig { folder_name: "sega_smd",  nointro_dat: "Sega - Mega Drive - Genesis.dat",     tgdb_platform_ids: &[18, 36] },
    SystemConfig { folder_name: "sega_gg",   nointro_dat: "Sega - Game Gear.dat",                tgdb_platform_ids: &[20] },
    SystemConfig { folder_name: "sega_sg",   nointro_dat: "Sega - SG-1000.dat",                  tgdb_platform_ids: &[4949] },
    SystemConfig { folder_name: "sega_32x",  nointro_dat: "Sega - 32X.dat",                      tgdb_platform_ids: &[33] },
    // Atari
    SystemConfig { folder_name: "atari_2600",   nointro_dat: "", tgdb_platform_ids: &[22] },
    SystemConfig { folder_name: "atari_5200",   nointro_dat: "", tgdb_platform_ids: &[26] },
    SystemConfig { folder_name: "atari_7800",   nointro_dat: "", tgdb_platform_ids: &[27] },
    SystemConfig { folder_name: "atari_jaguar", nointro_dat: "", tgdb_platform_ids: &[28] },
    SystemConfig { folder_name: "atari_lynx",   nointro_dat: "", tgdb_platform_ids: &[4924] },
    // NEC
    SystemConfig { folder_name: "nec_pce",   nointro_dat: "", tgdb_platform_ids: &[34] },
    SystemConfig { folder_name: "nec_pcecd", nointro_dat: "", tgdb_platform_ids: &[4955] },
    // Nintendo (no DAT yet)
    SystemConfig { folder_name: "nintendo_ds", nointro_dat: "", tgdb_platform_ids: &[8] },
    // SNK
    SystemConfig { folder_name: "snk_ng",   nointro_dat: "", tgdb_platform_ids: &[24] },
    SystemConfig { folder_name: "snk_ngcd", nointro_dat: "", tgdb_platform_ids: &[4956] },
    SystemConfig { folder_name: "snk_ngp",  nointro_dat: "", tgdb_platform_ids: &[4922, 4923] },
    // Disc-based consoles
    SystemConfig { folder_name: "sony_psx",      nointro_dat: "", tgdb_platform_ids: &[10] },
    SystemConfig { folder_name: "sega_dc",       nointro_dat: "", tgdb_platform_ids: &[16] },
    SystemConfig { folder_name: "sega_st",       nointro_dat: "", tgdb_platform_ids: &[17] },
    SystemConfig { folder_name: "sega_cd",       nointro_dat: "", tgdb_platform_ids: &[21] },
    SystemConfig { folder_name: "panasonic_3do", nointro_dat: "", tgdb_platform_ids: &[25] },
    SystemConfig { folder_name: "philips_cdi",   nointro_dat: "", tgdb_platform_ids: &[4917] },
    // Computer systems
    SystemConfig { folder_name: "amstrad_cpc",     nointro_dat: "", tgdb_platform_ids: &[4914] },
    SystemConfig { folder_name: "commodore_ami",   nointro_dat: "", tgdb_platform_ids: &[4911] },
    SystemConfig { folder_name: "commodore_amicd", nointro_dat: "", tgdb_platform_ids: &[4947] },
    SystemConfig { folder_name: "commodore_c64",   nointro_dat: "", tgdb_platform_ids: &[40] },
    SystemConfig { folder_name: "ibm_pc",          nointro_dat: "", tgdb_platform_ids: &[1] },
    SystemConfig { folder_name: "microsoft_msx",   nointro_dat: "", tgdb_platform_ids: &[4929] },
    SystemConfig { folder_name: "sharp_x68k",      nointro_dat: "", tgdb_platform_ids: &[4931] },
    SystemConfig { folder_name: "sinclair_zx",     nointro_dat: "", tgdb_platform_ids: &[4913] },
];
    ...
    if let Some(start) = name.find('(')
        && let Some(end) = name[start..].find(')') {
            ...
            let regions = [
                ...
                "Netherlands", "Russia", "Spain", "Sweden", "Taiwan", "UK",
            ];
...
fn parse_nointro_dat(path: &Path) -> Vec<NoIntroEntry> {
    ...
    for line in BufReader::new(file).lines() {
        ...
        if trimmed == ")" && in_rom { in_rom = false; continue; }
        ...
        if let Some(val) = extract_quoted_field(trimmed, "name ")
            && !in_rom && !trimmed.starts_with("rom (") && !trimmed.contains("rom ( name") {
                current_name = val;
            }
        ...
        if trimmed.starts_with("rom (") || trimmed.starts_with("rom(") {
            ...
            if trimmed.ends_with(')') { in_rom = false; }
        }
    }
    entries
}
...
fn parse_libretro_meta_dat(path: &Path, field_name: &str) -> HashMap<u32, String> {
    let file = match File::open(path) { Ok(f) => f, Err(_) => return HashMap::new() };
...
fn is_beta_or_proto(name: &str) -> bool {
    name.contains("(Beta") || name.contains("(Proto") || name.contains("(Sample") || name.contains("(Demo")
}
...
const CORPORATE_SUFFIXES: &[&str] = &[
    " Computer Entertainment Osaka", " Computer Entertainment Kobe", " Computer Entertainment Tokyo",
    " Digital Entertainment", " Technical Institute", " Interactive Software", " Entertainment",
    " Enterprises", " Corporation", " Industry", " of America", " of Japan",
    " Co., Ltd.", " Co., Ltd", " Corp.", " Corp", " LTD.", " Ltd.", " Ltd",
    " Inc.", " Inc", " Co.", " Co", " USA",
];
const REGIONAL_QUALIFIERS: &[&str] = &[" America", " Japan", " Europe", " do Brasil"];
const DIVISION_SUFFIXES: &[&str] = &[
    " AM1", " AM2", " AM3", " AM4", " AM5", " CS1", " CS2", " CS3",
    " R&D 1", " R&D 2", " R&D 3", " R&D 4", " R&D1", " R&D2", " R&D3", " R&D4",
    " EAD", " SPD",
];

fn is_noise(s: &str) -> bool {
    let lower = s.to_ascii_lowercase();
    lower == "bootleg" || lower == "<unknown>" || lower == "unknown"
        || lower.starts_with("bootleg ") || lower.starts_with("bootleg(")
        || lower.starts_with("hack ") || lower.starts_with("hack(") || lower == "hack"
}
...
fn normalize_case(s: &str) -> String {
    ...
    if alpha.is_empty() { return s.to_string(); }
    if alpha.iter().all(|c| c.is_ascii_uppercase()) && alpha.len() > 3 {
        let mut chars = s.chars();
        let first = chars.next().unwrap();
        let mut result = first.to_uppercase().to_string();
        for c in chars { result.extend(c.to_lowercase()); }
        result
    } else {
        s.to_string()
    }
...
fn normalize_developer(raw: &str) -> String {
    ...
    if let Some(canonical) = developer_override(trimmed) { return canonical.to_string(); }
    if is_noise(trimmed) { return String::new(); }
    ...
    if s.starts_with('[')
        && let Some(close) = s.find(']') {
            ...
            if !name.is_empty() { s = name; }
        }
    strip_suffixes_ci(&mut s, CORPORATE_SUFFIXES);
    strip_suffixes_ci(&mut s, REGIONAL_QUALIFIERS);
    if let Some(idx) = s.find(" / ") { s = s[..idx].trim().to_string(); }
    else if let Some(idx) = s.find('/') { s = s[..idx].trim().to_string(); }
    else if let Some(idx) = s.find(" + ") { s = s[..idx].trim().to_string(); }
    strip_suffixes_ci(&mut s, CORPORATE_SUFFIXES);
    strip_suffixes_ci(&mut s, REGIONAL_QUALIFIERS);
    ...
    if is_noise(&result) { String::new() } else { result }
}

fn tgdb_genre_name(id: u32) -> &'static str {
    match id {
        1 => "Action", 2 => "Adventure", 3 => "Board", 4 => "Card", 5 => "Casino",
        6 => "Comedy", 7 => "Compilation", 8 => "Shooter", 9 => "Educational",
        10 => "Family", 11 => "Fighting", 12 => "Horror", 13 => "MMO", 14 => "Music",
        15 => "Other", 16 => "Pinball", 17 => "Platform", 18 => "Puzzle", 19 => "Racing",
        20 => "Role-Playing", 21 => "Sandbox", 22 => "Simulation", 23 => "Sports",
        24 => "Stealth", 25 => "Strategy", 26 => "Trivia", 27 => "Virtual Life",
        28 => "Flight Simulator", 29 => "Fitness", 30 => "Party",
        _ => "",
    }
}
...
fn parse_tgdb_json(path: &Path) -> (HashMap<(String, u32), TgdbEntry>, TgdbRegionalDatesMap) {
    let file = match File::open(path) {
        Ok(f) => f,
        Err(e) => {
            eprintln!("Warning: could not open TheGamesDB JSON at {}: {}", path.display(), e);
    ...
    for game in games {
        let title = match game["game_title"].as_str() { Some(t) => t.to_string(), None => continue };
        let platform = match game["platform"].as_u64() { Some(p) => p as u32, None => continue };
        let release_date_raw = game["release_date"].as_str().unwrap_or("").to_string();
        let year: u16 = release_date_raw.get(..4).and_then(|y| y.parse().ok()).unwrap_or(0);
        let players: u8 = game["players"].as_u64().map(|p| p.min(255) as u8).unwrap_or(0);
        let genre_ids: Vec<u32> = game["genres"].as_array()
            .map(|a| a.iter().filter_map(|v| v.as_u64().map(|n| n as u32)).collect())
            .unwrap_or_default();
        let developer_ids: Vec<u32> = game["developers"].as_array()
            .map(|a| a.iter().filter_map(|v| v.as_u64().map(|n| n as u32)).collect())
            .unwrap_or_default();
        let publisher_ids: Vec<u32> = game["publishers"].as_array()
            .map(|a| a.iter().filter_map(|v| v.as_u64().map(|n| n as u32)).collect())
            .unwrap_or_default();
        ...
        let alternates: Vec<String> = game["alternates"].as_array()
        ...
        if year > 0 && !release_date_raw.is_empty() {
            ...
            regional_dates.entry(key.clone()).or_default().push(TgdbRegionalDate {
                region_id,
        ...
        result.entry(key).or_insert(TgdbEntry {
            title, year, players, genre_ids, developer_ids, publisher_ids, coop, rating, alternates,
        });
    }
...
fn normalize_title_for_tgdb(title: &str) -> String {
    ...
    for ch in title.chars() {
        if ch.is_alphanumeric() || ch == ' ' { result.push(ch.to_ascii_lowercase()); }
    }
...
fn insert_console_games(conn: &Connection, sources_dir: &Path) -> rusqlite::Result<()> {
    ...
    eprintln!("Game DB: TGDB lookups: {} devs, {} pubs, {} genres", tgdb_developers.len(), tgdb_publishers.len(), tgdb_genres.len());

    let mut total_roms = 0usize;
    let mut total_games = 0usize;
    let mut total_tgdb_matches = 0usize;
    let mut console_release_dates: Vec<(String, String, &'static str, String, &'static str)> = Vec::new();
    ...
    for sys in GAME_DB_SYSTEMS {
        ...
        if !dat_path.exists() {
            eprintln!("Game DB: No-Intro DAT not found for {}, skipping", sys.folder_name);
            continue;
        }

        let nointro_entries = parse_nointro_dat(&dat_path);
        eprintln!("Game DB: {} - parsed {} ROM entries", sys.folder_name, nointro_entries.len());
        ...
        for group_key in &group_keys {
            ...
            for &platform_id in sys.tgdb_platform_ids {
                if let Some(region_rows) = tgdb_regional_dates.get(&(tgdb_normalized.clone(), platform_id)) {
            ...
            for &platform_id in sys.tgdb_platform_ids {
                if let Some(tgdb_entry) = tgdb.get(&(tgdb_normalized.clone(), platform_id)) {
                    ...
                    if !tgdb_entry.genre_ids.is_empty() {
                        tgdb_genre = tgdb_genres.get(&tgdb_entry.genre_ids[0]).cloned()
                            .unwrap_or_else(|| tgdb_genre_name(tgdb_entry.genre_ids[0]).to_string());
                    }
            ...
            for &idx in indices {
                if is_beta_or_proto(&nointro_entries[idx].name) { continue; }
                let crc = nointro_entries[idx].crc32;
                if final_players == 0
                    && let Some(s) = maxusers.get(&crc) { final_players = s.parse().unwrap_or(0); }
                if final_genre.is_empty()
                    && let Some(g) = genres.get(&crc) { final_genre = g.clone(); }
                if final_players > 0 && !final_genre.is_empty() { break; }
            }
            ...
            if final_players == 0 { final_players = tgdb_players; }
            if final_genre.is_empty() { final_genre = tgdb_genre; }
            ...
            for &idx in indices {
                ...
                let stem = entry.rom_filename.rfind('.')
        ...
        eprintln!(
            ...
            sys.folder_name, canonical_games.len(), rom_entries.len(), tgdb_match_count
        );
        ...
        for entry in &rom_entries {
            ...
            if !seen_stems.insert(stem) { continue; }
    ...
    eprintln!("Game DB: Inserted {} console release date rows", console_release_dates.len());
...
fn insert_series(conn: &Connection, sources_dir: &Path) -> rusqlite::Result<()> {
    ...
    let mut entries: Vec<WikidataSeriesEntryRaw> = match serde_json::from_reader(BufReader::new(file)) {
        Ok(v) => v,
        Err(e) => {
    ...
    // Reverse-link pass
    {
        let norm_to_idx: HashMap<String, Vec<usize>> = {
            ...
            for (i, entry) in entries.iter().enumerate() {
                ...
                if !norm.is_empty() { map.entry(norm).or_default().push(i); }
            }
            map
        };
        ...
        for (idx, follows_fix, followed_by_fix, series_fix) in fixes {
            if let Some(f) = follows_fix { entries[idx].follows = Some(f); }
            if let Some(f) = followed_by_fix { entries[idx].followed_by = Some(f); }
            if let Some(s) = series_fix { entries[idx].series_name = Some(s); }
        }
    }
    ...
    for entry in &entries {
        ...
        if series_name.is_empty() && follows.is_empty() && followed_by.is_empty() { continue; }
...
fn main() {
    ...
    conn.execute("INSERT INTO db_meta (key, value) VALUES ('mame_version', ?1)", params!["0.285"])
        .expect("Failed to insert mame_version");
    conn.execute("INSERT INTO db_meta (key, value) VALUES ('generated_at', ?1)", params![generated_at])
        .expect("Failed to insert generated_at");
    conn.execute("INSERT INTO db_meta (key, value) VALUES ('is_stub', ?1)", params![is_stub])
        .expect("Failed to insert is_stub");

    // Final stats
    let arcade_count: i64 = conn.query_row("SELECT COUNT(*) FROM arcade_games", [], |r| r.get(0)).unwrap_or(0);
    let canonical_count: i64 = conn.query_row("SELECT COUNT(*) FROM canonical_games", [], |r| r.get(0)).unwrap_or(0);
    let rom_count: i64 = conn.query_row("SELECT COUNT(*) FROM rom_entries", [], |r| r.get(0)).unwrap_or(0);
    let series_count: i64 = conn.query_row("SELECT COUNT(*) FROM series_entries", [], |r| r.get(0)).unwrap_or(0);
