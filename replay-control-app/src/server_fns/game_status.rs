...
pub use replay_control_core::user_data::{GameStatus, StatusGameEntry};
...
pub async fn set_game_status(
    ...
    state
        .user_data_writer
        .try_write(move |conn| UserDataDb::set_game_status(conn, &system, &rom_filename, status))
        .await
...
pub async fn clear_game_status(
    ...
    state
        .user_data_writer
        .try_write(move |conn| UserDataDb::clear_game_status(conn, &system, &rom_filename))
        .await
        .map_err(|e| ServerFnError::new(e.to_string()))?
        .map_err(|e| ServerFnError::new(e.to_string()))?;
    Ok(())
...
pub async fn get_games_by_status(
    status: GameStatus,
) -> Result<Vec<StatusGameEntry>, ServerFnError> {
    let state = expect_context::<crate::api::AppState>();
