...
fn get_ra_credentials(state: &crate::api::AppState) -> Option<(String, String)> {
    // Web API key is stored on its own (read-only RA Web API), while the
    // username is reused from the existing RetroAchievements (rcheevos) config.
    let config = state.settings.load();
