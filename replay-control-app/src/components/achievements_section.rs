...
use crate::server_fns::{self, GameStatus, RaGameInfo};
...
pub fn AchievementsSection(
    ...
    let _sync = Effect::new(move || {
        if let Some(result) = achievements_resource.get() {
            match result {
                Ok(info) => {
                    let has_progress = info.as_ref().map(|g| g.earned_count > 0).unwrap_or(false);
                    let is_complete = info.as_ref().map(|g| g.is_complete).unwrap_or(false);
                    game_info.set(info);
                    loaded.set(true);
                    has_user_progress.set(has_progress);

                    if is_complete && current_status.get_untracked() != Some(GameStatus::Platinum) {
                        current_status.set(Some(GameStatus::Platinum));
                        let sys = system.get_value();
                        let fname = rom_filename.get_value();
                        leptos::task::spawn_local(async move {
                            let _ = server_fns::set_game_status(sys, fname, GameStatus::Platinum).await;
                        });
                    }
                }
                Err(_) => {
