...
pub fn MyGamesPage() -> impl IntoView {
    ...
    let games = RwSignal::new(Vec::<StatusGameEntry>::new());

    let _sync = Effect::new(move || {
        if let Some(Ok((_, result))) = games_resource.get() {
            games.set(result);
        }
    });
        ...
        <div class="page my-games-page">
            ...
            <Suspense fallback=move || view! { <div class="my-games-loading">{move || t(i18n.locale.get(), Key::CommonLoading)}</div> }>
                {move || Suspend::new(async move {
                    let _ = games_resource.await;
                    Ok::<_, ServerFnError>(view! {
                        <Show when=move || !games.read().is_empty()
                            fallback=move || view! {
                                <div class="my-games-empty">
                                    {move || t(i18n.locale.get(), Key::MyGamesEmpty)}
                                </div>
                            }
                        >
                            <div class="my-games-grid">
                                {move || {
                                    games.get().into_iter().map(|entry| {
                                        view! {
                                            <GameStatusCard entry=entry />
                                        }
                                    }).collect::<Vec<_>>()
                                }}
                            </div>
