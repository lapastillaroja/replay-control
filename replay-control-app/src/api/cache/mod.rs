...
impl GameLibrary {
    ...
    /// Get a page of ROMs for a system with SQL-level pagination fallback.
    ///
    /// - If L1 cache is warm, paginates in memory (current fast path).
    /// - If L1 is cold, does a direct paginated L2 (SQLite) query instead of
    ///   loading the entire system into memory first.
    ///
    /// Returns `Some((page_entries, total_count))`, or `None` if neither L1 nor L2
    /// has data (caller should fall back to the full `get_roms()` path).
    pub async fn get_roms_page_direct(
        &self,
        storage: &StorageLocation,
        system: &str,
        offset: usize,
        limit: usize,
    ) -> Option<(Vec<RomEntry>, usize)> {
        let system_dir = storage.roms_dir().join(system);

        // L1: in-memory cache — paginate from the Arc.
        if let Ok(guard) = self.roms.read()
            ...
            && entry.is_fresh(&system_dir)
        {
            let all = &entry.data;
            let total = all.len();
            let page: Vec<RomEntry> = all.iter().skip(offset).take(limit).cloned().collect();
            return Some((page, total));
        }

        // L2: direct paginated SQLite query (avoids loading all ROMs).
        let sys = system.to_string();
        let count = self
            .db
            .read({
                let sys = sys.clone();
                move |conn| MetadataDb::count_system_entries(conn, &sys)
            })
            .await?
            .ok()?;
        ...
        let entries = self
            ...
            .read({
                let sys = sys.clone();
                move |conn| MetadataDb::load_system_entries_page(conn, &sys, offset, limit)
            })
            .await?
            .ok()?;
        ...
        tracing::debug!(
            "L2 paginated query for {system}: offset={offset}, limit={limit}, total={count}, returned={}",
            roms.len()
        );
