...
//! `background.rs` — it's a cohesive concern that shares no state with the
//! library scan/enrichment pipeline that fills the rest of that file. The
//! methods stay on `BackgroundManager` (a second `impl` block); only the
//! source location moves.
...
use super::AppState;
use super::background::{BackgroundManager, env_duration_secs};
