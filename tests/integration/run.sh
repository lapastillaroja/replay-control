...
assert_status "Game Library page returns 200" "$BASE/settings/game-library" "200"
assert_contains "Game Library page mentions Built-in" "$BASE/settings/game-library" "Built"
...
COLD=$(curl -s -o /dev/null -w "%{time_total}" "$BASE/settings/game-library" 2>/dev/null || echo "0")
WARM=$(curl -s -o /dev/null -w "%{time_total}" "$BASE/settings/game-library" 2>/dev/null || echo "0")
