...
if [[ "${SKIP_DATA:-}" == "1" ]]; then
    ...
    if [[ "${BUILD_CATALOG_STUB:-}" == "1" ]]; then
        # Build from the committed replay-control-core/fixtures/ tree instead
        # of data/upstream — no downloads, no third-party hosts, no TGDB key.
        # Only a handful of games, so this validates that the build pipeline
        # itself works (compiles, links, wasm-opts); it does not validate real
        # catalog coverage. See test-full.yml for the full-data build that does.
        echo "==> BUILD_CATALOG_STUB=1: building catalog from replay-control-core/fixtures (no downloads)."
        catalog_args+=(--stub)
    else
