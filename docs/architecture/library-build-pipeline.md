...
2. Resolve M3U containers before row materialization. Disc playlists and
...
flowchart TD
    ...
    O --> P["Compute/reuse hash outside writer"]
...
3. A bounded worker set claims 200-row mini-batches by marking those rows
   `Running`.
4. Workers compute or reuse CRCs/runtime hashes outside the SQLite writer closure.
...
manual rebuild because forced hash work was on the blocking path.
