...
| `regenerate_metadata` clear (E7) ↔ concurrent `Activity::Rebuild` reading enrichment (L4) | Yes — E7 clears `launchbox_game` without claiming activity, then *tries* to claim `RefreshExternalMetadata` | If the spawn-claim fails (busy), launchbox tables are gone and re-enrichment silently runs against an empty source | **Gap F-2** |
