...
4. **Bundled `catalog.sqlite` — catalog_game_resource**: MiSTer Manual Downloader and Retrokit manual links. Matching resources are copied into `library_game_resource` during enrichment; saving a manual downloads the file into user-owned storage.
