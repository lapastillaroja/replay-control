...
- **SQLite game library** — persistent L2 cache (`game_library` + `game_library_meta` tables in `metadata.db`) between in-memory L1 and filesystem L3. Eliminates cold-start penalties. Write-through on L3 scan, mtime-based invalidation.
- **Game recommendations** — home page recommendation blocks: random picks, top genres, multiplayer count, favorites-based picks, top-rated games. Powered by SQLite game library queries (no per-ROM filesystem I/O). See `docs/reference/recommendations-optimization-plan.md`.
