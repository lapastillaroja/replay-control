...
> **Status**: Implemented. The `arcade_db` module (`replay-control-core/src/arcade_db.rs`) contains ~28K entries as a compile-time PHF map, generated by `build.rs`. Includes display name, year, manufacturer, players, rotation, driver status, clone/parent info, and category.
