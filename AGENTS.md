...
- When simplifying or reviewing for cleanup (e.g. `/simplify`), reach for the structural refactor, not just local tidying. If the real win is moving a type to the right layer, collapsing duplicated logic into one helper, threading a struct instead of a loose tuple of args, or unifying a surface that diverges from its siblings, **do it** — or, when it's genuinely out of scope or risky, **propose it explicitly** rather than silently settling for the timid local edit. Treat a surface that diverges from how all its siblings work as a smell to investigate, not a "tradeoff" to excuse; the burden is on the exception to justify itself. A cleanup pass that only deletes a few comments while a deeper reuse/altitude win sits unaddressed has under-delivered.
