...
impl LibraryDb {
            ...
            .query_row(
                ...
                |row| {
                    Ok(GameMetadata {
                        ...
                        release_precision: row.get::<_, Option<DpSql>>(8)?.map(|DpSql(d)| d),
            ...
            .query_map(params![system], |row| {
                Ok((
                    ...
                    GameMetadata {
                        ...
                        release_precision: row.get::<_, Option<DpSql>>(9)?.map(|DpSql(d)| d),
    ...
    pub fn upsert(
        ...
        conn.execute(
                ...
                params![
                    ...
                    meta.release_date,
                    meta.release_precision.map(DpSql),
    ...
    pub fn stats(conn: &Connection, db_path: &Path) -> Result<MetadataStats> {
        ...
        let last_updated_text = conn
            .query_row("SELECT MAX(imported_at) FROM data_sources", [], |row| {
                row.get::<_, i64>(0)
            })
            .ok()
            .map(|ts| {
    ...
    pub fn bulk_update_image_paths(
        ...
        let mut count = 0usize;
        {
            ...
            let mut insert_stmt = tx
                .prepare(
                    "INSERT OR IGNORE INTO game_metadata (system, rom_filename, fetched_at, box_art_path, screenshot_path, title_path)
                     VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
                )
